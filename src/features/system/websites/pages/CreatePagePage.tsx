...
export const CreatePagePage: FC<CreatePagePageProps> = ({ websiteId }) => {
  ...
  const createPageMutation = websitesService.useCreatePage()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(t('messages.pageCreateSuccess'))
      navigate({ to: `/${locale}/websites/${websiteId}/pages/${result}` })
    } catch (error: any) {
      handleError(error)
    }
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">{t('pages.create')}</h1>
          <p className="text-gray-600 mt-2">Create a new page for {website.name}</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-3 justify-end pt-4 border-t">
              ...
              <Button
                ...
                disabled={createPageMutation.isPending}
              >
                {createPageMutation.isPending ? t('form.buttons.saving') : t('form.buttons.save')}
              </Button>
            </div>
          </form>
        </Card>
