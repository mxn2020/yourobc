// src/features/boilerplate/lib/auth-config.ts - IN DEVELOPMENT
...
async function syncUserToConvex(user: AuthUser, action: 'create' | 'update' | 'sync' = 'sync') {
  ...
  try {
    ...
    await convex.mutation(api.lib.boilerplate.user_profiles.mutations.syncProfileFromAuth, {
      authUserId: userId,
      email: user.email,
...
async function trackUserLogin(session: AuthSession) {
  try {
    ...
    await convex.mutation(api.lib.boilerplate.user_profiles.mutations.updateActivity, {
      activityType: 'login',
      metadata: {
...
export async function recoverAllUsers() {
  ...
  try {
    ...
    const missingCheck = await convex.query(api.lib.boilerplate.user_profiles.recovery.findMissingProfiles, {
      authUserIds
    });
    ...
    if (missingCheck.missing > 0) {
      ...
      const results = await convex.mutation(api.lib.boilerplate.user_profiles.recovery.batchRecoverProfiles, {
