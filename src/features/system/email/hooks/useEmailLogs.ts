...
export function useEmailLogs(filters?: {
  ...
  return useConvexQuery(api.lib.system.email.queries.getEmailLogs, filters || {});
}
...
export function useEmailLogByMessageId(messageId: string) {
  return useConvexQuery(api.lib.system.email.queries.getEmailLogByMessageId, { messageId });
}
...
export function useEmailLogById(logId: Id<"emailLogs">) {
  return useConvexQuery(api.lib.system.email.queries.getEmailLogById, { logId });
}
...
export function useEmailStats(days = 30) {
  return useConvexQuery(api.lib.system.email.queries.getEmailStats, { days });
}
