// src/features/boilerplate/admin/services/PermissionRequestsService.ts
...
class PermissionRequestsService {
  ...
  getPermissionRequestsStatsQueryOptions() {
    return convexQuery(api.lib.boilerplate.permission_requests.queries.getPermissionRequestsStats, {});
  }

  getAllPermissionRequestsQueryOptions(status?: 'pending' | 'approved' | 'denied') {
    return convexQuery(api.lib.boilerplate.permission_requests.queries.getAllPermissionRequests, {
      status,
    });
  }

  getUserPermissionRequestsQueryOptions(userId: Id<'userProfiles'>) {
    return convexQuery(api.lib.boilerplate.permission_requests.queries.getUserPermissionRequests, {
      userId,
    });
  }
  ...
  useApprovePermissionRequest() {
    return useMutation({
      mutationFn: useConvexMutation(api.lib.boilerplate.permission_requests.mutations.approvePermissionRequest),
    });
  }

  useDenyPermissionRequest() {
    return useMutation({
      mutationFn: useConvexMutation(api.lib.boilerplate.permission_requests.mutations.denyPermissionRequest),
    });
  }

  useCreatePermissionRequest() {
    return useMutation({
      mutationFn: useConvexMutation(api.lib.boilerplate.permission_requests.mutations.createPermissionRequest),
    });
  }

  useCancelPermissionRequest() {
    return useMutation({
      mutationFn: useConvexMutation(api.lib.boilerplate.permission_requests.mutations.cancelPermissionRequest),
    });
  }
