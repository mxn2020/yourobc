...
class AuditLogsService {
  ...
  useEntityAuditLogs(entityType: AuditEntityType, entityId: string, limit?: number) {
    return useQuery({
      ...convexQuery(api.lib.system.core.audit_logs.queries.getEntityAuditLogs, {
        entityType,
        entityId,
        limit,
      }),
  ...
  useUserAuditLogs(limit?: number) {
    return useQuery({
      ...convexQuery(api.lib.system.core.audit_logs.queries.getUserAuditLogs, {
        limit,
      }),
      staleTime: 60000,
    })
  ...
  useCreateAuditLog() {
    return useMutation({
      mutationFn: useConvexMutation(api.lib.system.core.audit_logs.mutations.createAuditLog),
    })
  }
