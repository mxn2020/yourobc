...
export const createWebhook = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const webhookId = await ctx.db.insert('webhooks', {
      ...
      timeout: args.timeout || INTEGRATIONS_CONSTANTS.CONFIG.WEBHOOK_TIMEOUT_MS,
      retryConfig: args.retryConfig || {
        enabled: true,
        maxAttempts: INTEGRATIONS_CONSTANTS.CONFIG.WEBHOOK_MAX_RETRIES,
        backoffMultiplier: INTEGRATIONS_CONSTANTS.CONFIG.WEBHOOK_RETRY_BACKOFF_MULTIPLIER,
        initialDelay: INTEGRATIONS_CONSTANTS.CONFIG.WEBHOOK_INITIAL_RETRY_DELAY_MS,
      },
...
export const createOAuthApp = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const appId = await ctx.db.insert('oauthApps', {
      ...
      rateLimit: {
        requestsPerMinute: INTEGRATIONS_CONSTANTS.CONFIG.DEFAULT_RATE_LIMITS.REQUESTS_PER_MINUTE,
        requestsPerHour: INTEGRATIONS_CONSTANTS.CONFIG.DEFAULT_RATE_LIMITS.REQUESTS_PER_HOUR,
      },
      isActive: true,
      isVerified: false,
      totalUsers: 0,
  ...
  handler: async (ctx, args) => {
    ...
    const tokenId = await ctx.db.insert('oauthTokens', {
      ...
      expiresAt: calculateTokenExpiration(
        INTEGRATIONS_CONSTANTS.CONFIG.OAUTH_ACCESS_TOKEN_EXPIRATION_SECONDS
      ),
      refreshTokenExpiresAt: calculateTokenExpiration(
        INTEGRATIONS_CONSTANTS.CONFIG.OAUTH_REFRESH_TOKEN_EXPIRATION_SECONDS
      ),
      isRevoked: false,
      usageCount: 0,
      createdAt: now,
...
export const exchangeAuthorizationCode = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    return {
      ...
      refreshToken,
      tokenType: 'Bearer',
      expiresIn: INTEGRATIONS_CONSTANTS.CONFIG.OAUTH_ACCESS_TOKEN_EXPIRATION_SECONDS,
    };
  },
});
