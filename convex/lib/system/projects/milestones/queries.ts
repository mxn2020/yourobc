// convex/lib/system/milestones/queries.ts
...
import { requireViewAccess, filterMilestonesByAccess } from './permissions';
...
export const getMilestone = query({
  ...
  handler: async (ctx, { milestoneId }) => {
    ...
    await requireViewAccess(ctx, milestone, user);
...
export const getProjectMilestones = query({
  ...
  handler: async (ctx, { projectId }) => {
    ...
    const milestones = await ctx.db
      .query('projectMilestones')
      .withIndex('by_project_due_date', (q) => q.eq('projectId', projectId))
