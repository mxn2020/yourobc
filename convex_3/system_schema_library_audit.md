...
- `convex/schema/system/core/system_metrics/systemMetrics.ts`: No `ownerId`; custom audit fields with optional `createdBy`; uses nested-field indexes and lacks standard audit fields from `auditFields`.
- `convex/schema/system/core/analytics/analytics.ts`: `analyticsEventsTable` and `analyticsMetricsTable` lack a display field (`name/title/displayName`) and do not index it; several indexes use deprecated names (`by_created`, `by_deleted`); no required `by_public_id` display index for events/metrics.
...
- `convex/schema/system/core/permission_requests/permissionRequests.ts`: Custom audit fields instead of shared `auditFields`; misses standardized audit/soft-delete spread pattern.
